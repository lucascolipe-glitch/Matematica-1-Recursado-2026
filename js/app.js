...
const CONFIG = {
  reales: {
    ...
    casos: [
      {id:'R-TP-01', titulo:'Clasificar antes de operar', enunciado:'¿Cuál es la clasificación más específica de \\(\\sqrt{7}\\)?', opciones:['Racional','Irracional','Entero'], correcta:1, bien:'\\(\\sqrt{7}\\) no puede escribirse como cociente de enteros. Es irracional y, por inclusión, también real.', mal:'No toda raíz es irracional, pero 7 no es un cuadrado perfecto. La clasificación más específica es irracional.'},
      {id:'R-TP-02', titulo:'Propiedades y prioridades', enunciado:'Calculá \\(\\frac{2^5}{2^3}+3^{-1}-\\sqrt{2}\\,\\sqrt{2}+\\left(\\frac12\\right)^{-2}\\).', opciones:['\\(\\frac{19}{3}\\)','\\(\\frac{13}{3}\\)','\\(6\\)'], correcta:0, bien:'\\(2^2+\\frac13-2+4=6+\\frac13=\\frac{19}{3}\\). La clave es aplicar las propiedades antes de sumar.', mal:'Revisá cuatro pasos: cociente de potencias, exponente negativo, producto de raíces y prioridad de operaciones.'},
      {id:'R-TP-03', titulo:'Riesgo y logaritmo decimal', enunciado:'En \\(R=\\log_{10}(PDC)\\), con \\(P=0{,}001\\), \\(D=100\\) y \\(C=0{,}1\\), ¿cuál es \\(R\\)?', opciones:['−2','2','−1'], correcta:0, bien:'El producto es \\(0{,}01=10^{-2}\\); por definición, \\(\\log_{10}(0{,}01)=-2\\).', mal:'Calculá primero el producto y escribilo como potencia de 10.'}
    ],
    videos: [
  ...
  ecuaciones: {
    ...
    casos: [
      {id:'E-TP-01', titulo:'Eliminar denominadores con uniformidad', enunciado:'Resolvé \\(\\frac{3a-2}{3}+\\frac{a-3}{2}=\\frac56\\).', opciones:['\\(a=2\\)','\\(a=-2\\)','\\(a=1\\)'], correcta:0, bien:'Multiplicar ambos miembros por \\(6\\) da \\(2(3a-2)+3(a-3)=5\\); de allí \\(9a-13=5\\) y \\(a=2\\).', mal:'Usá 6 como denominador común y aplicá la multiplicación a todos los términos.'},
      {id:'E-TP-02', titulo:'Simplificar sin olvidar la restricción', enunciado:'¿Cuál es la solución de \\(\\frac{(x+1)(x-1)}{x-1}=0\\)?', opciones:['\\(x=-1\\)','\\(x=1\\)','\\(x=-1\\) o \\(x=1\\)'], correcta:0, bien:'La condición inicial es \\(x\\ne1\\). Al simplificar queda \\(x+1=0\\), por lo tanto \\(x=-1\\).', mal:'Calculá la condición inicial antes de cancelar x−1.'},
      {id:'E-TP-03', titulo:'Identidad con condiciones iniciales', enunciado:'En \\(\\frac{1}{x-1}+\\frac{2}{x-3}=\\frac{3x-5}{(x-1)(x-3)}\\), ¿cuál es el conjunto solución?', opciones:['\\(\\mathbb{R}\\)','\\(\\mathbb{R}\\setminus\\{1,3\\}\\)','\\(\\varnothing\\)'], correcta:1, bien:'La igualdad se reduce a una identidad, pero \\(x=1\\) y \\(x=3\\) estaban excluidos por los denominadores.', mal:'Una identidad no borra las condiciones iniciales de la expresión original.'}
    ],
    videos: [
  ...
  inecuaciones: {
    ...
    casos: [
      {id:'I-TP-01', titulo:'Inecuación lineal', enunciado:'Resolvé \\(2(x+1)-3(x-2)<x+1\\).', opciones:['\\(x>\\frac72\\)','\\(x<\\frac72\\)','\\(x>7\\)'], correcta:0, bien:'La desigualdad queda \\(-x+8<x+1\\), luego \\(7<2x\\) y \\(x>\\frac72\\).', mal:'Aplicá distributiva, agrupá x en un miembro y controlá el signo del factor por el que dividís.'},
      {id:'I-TP-02', titulo:'Producto y signos', enunciado:'Resolvé \\((2x-3)(x+1)\\ge0\\).', opciones:['\\([-1,\\frac32]\\)','\\((\\! -\\infty,-1]\\cup[\\frac32,\\infty)\\)','\\((\\! -\\infty,-1)\\cup(\\frac32,\\infty)\\)'], correcta:1, bien:'El producto es no negativo fuera de las raíces y las incluye porque el símbolo es \\(\\ge\\).', mal:'Marcá las raíces \\(-1\\) y \\(\\frac32\\), analizá signos por intervalos y revisá si los extremos se incluyen.'},
      {id:'I-TP-03', titulo:'Valor absoluto como intervalo', enunciado:'Resolvé \\(|3x+4|\\le1\\).', opciones:['\\([-\\frac53,-1]\\)','\\([-1,\\frac53]\\)','\\((\\! -\\infty,-\\frac53]\\cup[-1,\\infty)\\)'], correcta:0, bien:'\\(-1\\le3x+4\\le1\\) conduce a \\(-5\\le3x\\le-3\\) y finalmente \\(-\\frac53\\le x\\le-1\\).', mal:'Usá la propiedad del menor: convertí el valor absoluto en una doble desigualdad.'}
    ],
    videos: [
...
// MathJax --------------------------------------------------------------------
// Reprocesa únicamente la zona que cambió. Si MathJax no cargó, la cápsula
// sigue funcionando y no genera errores de consola.
function limpiarMathJax(elementos) {
  if (window.MathJax?.typesetClear) {
    try { window.MathJax.typesetClear(elementos.filter(Boolean)); } catch (error) { console.warn('No se pudo limpiar MathJax:', error); }
  }
}
function renderizarMathJax(elemento = document) {
  ...
  const ejecutar = () => {
    ...
    return window.MathJax.typesetPromise([elemento]).catch(error => console.warn('No se pudo renderizar MathJax:', error));
  };
  return window.MathJax.startup?.promise ? window.MathJax.startup.promise.then(ejecutar) : ejecutar();
}
...
function iniciarCapsula() {
  ...
  function mostrarPantalla(id) {
    ...
    try {
      ...
      // Respaldo para navegadores que restringen replaceState con archivos locales.
      location.hash = id;
    }
    ...
    if (id === 'audiovisual') renderVideo(true);
  ...
  document.querySelectorAll('[data-microquiz]').forEach(bloque => {
    bloque.addEventListener('click', e => {
      ...
      const devolucion = bloque.querySelector('.devolucion');
      limpiarMathJax([devolucion]);
      devolucion.textContent = elegida===correcta ? bloque.dataset.bien : bloque.dataset.mal;
  ...
  function renderCaso() {
    ...
    limpiarMathJax([elEnunciado, elOpciones, elDev]);
    ...
    renderizarMathJax(document.querySelector('.caso-card'));
  }
  ...
  // YouTube ------------------------------------------------------------------
  function renderVideo(cargarIframe=false){
    const v=videos[estado.video];
    const iframe=document.getElementById('youtube-iframe');
    if(!v||!iframe)return;
    const titulo=document.getElementById('youtube-titulo');
    const descripcion=document.getElementById('youtube-descripcion');
    const enlace=document.getElementById('youtube-enlace');
    if(titulo)titulo.textContent=v.titulo;
    if(descripcion)descripcion.textContent=v.descripcion;
    if(enlace)enlace.href=`https://www.youtube.com/watch?v=${v.id}`;
    if(cargarIframe)iframe.src=`https://www.youtube-nocookie.com/embed/${v.id}`;
  }
  document.getElementById('youtube-siguiente')?.addEventListener('click',()=>{estado.video=(estado.video+1)%videos.length;renderVideo(true);});
  document.getElementById('youtube-anterior')?.addEventListener('click',()=>{estado.video=(estado.video-1+videos.length)%videos.length;renderVideo(true);});
  renderVideo(false);
  renderizarMathJax(document.body);
}
